					...
					dbc.elements.forEach(function (element, dbcIndex, dbcElements) {
						if (element.name == abb.View) {
							if (abb.View === "Expected Public Value & Estimated Budget") {
								...
								let additionalInfo = {
									name: dbc.Name.concat(' - ', dbc.ID,' - ', abb.View, ' - Additional-Info'),
									type: "panel",
								...
								// Requisito
								let newRequirement = {
									name: dbc.Name.concat(' - ', dbc.ID,' - ', abb.View, '-', index++),
						...
						} else if (element.name === "Expected Public Value & Estimated Budget" && element.elements.length == 0) {
							let radiogroupElement = {
								name: dbc.Name.concat(' - ', dbc.ID, ' - ', 'Expected Public Value'),
								title: " ",
								type: "radiogroup",
								colCount: 0,
								choices: [
							  ...
							  panel1.elements.push(radiogroupElement);
	  
							  let panel2 = {
								type: "text",
								name: dbc.Name.concat(' - ', dbc.ID, ' - ', 'Estimated Budget (mill €)'),
							  };
		
								element.elements.push(panel1);
								element.elements.push(panel2);
						}
					});
				});
			});
		...
		dbcList.forEach(
            ...
            <p style="font-size:20px;">The DBC ${dbc.Description}</p>`
				...
				pages.push({
					id: `${dbc.ID}`,
					name: dbc.Name,
					visibleIf: `{${dbc.Name} - ${dbc.ID} - Include} contains 'Include'`,
...
function extractDpsFromDbc(DBC, DpsList, abbList) {
	...
	DBC.Digital_Public_Services.forEach(dbcDps => {
		...
		let scoreName = DBC.Name.concat(' - ', DBC.ID,' - ', extractedDps.Name, scoreIndex, ' - Score');
		...
		processedDps.push({
			...
			processedDps.push({
				...
				{
					name: DBC.Name.concat(' - ', DBC.ID,' - ', extractedDps.Name, ' - Comments'),
	...
	processedDps.push({
		name: DBC.Name.concat(' - ', DBC.ID,' - Summary'),
		type: "html",
	...
	processedDps.push({
		name: DBC.Name.concat(' - ', DBC.ID,' - Target Prospective Ability'),
		type: "text",
		inputType: "range",
	...
	processedDps.push({
		name: DBC.Name.concat(' - ', DBC.ID, ' - Gap Difference'),
		type: "expression",
		...
		expression: "{".concat(DBC.Name,' - ', DBC.ID, ' - Total Score', "} - {", DBC.Name,' - ', DBC.ID, ' - Target Prospective Ability', "}"),
		visible: true
	});
